...
export default function CuentaRegresiva({ iso }: { iso: string }) {
    ...
    <div className="flex gap-2.5 sm:gap-3" aria-live="off">
      {UNIDADES.map((u, i) => (
        <motion.div
          ...
          className="min-w-[62px] flex-1 border border-white/15 bg-white/[0.04] px-2 py-3 text-center sm:min-w-[74px]"
        >
          <span className="block font-heading text-3xl leading-none tabular-nums sm:text-4xl">
            {t ? String(t[u.clave]).padStart(2, "0") : "––"}
          </span>
          <span className="mt-1.5 block font-mono text-[10px] uppercase tracking-[0.2em] text-zinc-500">
            {u.label}
          </span>
        </motion.div>
      ))}
